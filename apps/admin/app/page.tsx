...
export default async function AdminHomePage() {
  await requireAdmin();
  const [products, status] = await Promise.all([listReviewRecords(), getOperationalControlSurface()]);
    ...
    <div style={{ display: 'grid', gap: 20 }}>
      <section style={{ background: '#fff', border: '1px solid rgba(0,0,0,0.08)', borderRadius: 20, padding: 20 }}>
        ...
        <p style={{ margin: '12px 0 0', fontSize: 15, lineHeight: 1.6, color: 'rgba(0,0,0,0.72)' }}>
          This DB-backed admin surface exposes provenance, freshness, confidence, and review state before any listing is eligible for storefront visibility.
        </p>
      </section>
